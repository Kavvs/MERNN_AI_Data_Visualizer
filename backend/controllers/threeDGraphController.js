...
exports.generate3DGraph = async (req, res) => {
  try {
    ...
    // 2. Create 3D bars from data
    data.forEach((item, index) => {
      const height = item.age || 1; // Default to 1 if age missing
      const geometry = new BoxGeometry(1, height, 1);
      const material = new MeshBasicMaterial({ 
        color: Math.random() * 0xffffff // Random colors
      });
    ...
    // 3. Render to PNG (using node-three)
    const { PNG } = require('pngjs');
    const { createCanvas } = require('canvas');
    const canvas = createCanvas(800, 600);
    const ctx = canvas.getContext('2d');
    
    // Simulate rendering (for server-side)
